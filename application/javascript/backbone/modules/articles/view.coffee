...
#     didClickMore: (event) ->
#       do event.preventDefault
#       query = if @model.get('kind') is 'tweet' then '@' + @model.get('user').username else @model.get('domain')
...
#       if title.length > 95 and @model.get('kind') isnt 'tweet' then title = title.substring(0, 94) + '&hellip;'
